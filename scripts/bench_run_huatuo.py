"""HuatuoGPT-Vision-7B benchmark runner with multi-GPU sharding.

Loads HuatuoGPT-Vision-7B (Qwen2-7B base + their LLaVA fork) on a single GPU
and processes a hash-shard of the probes. Run multiple instances in parallel
(one per GPU) to use all 6 GPUs.

Usage (one process per GPU):
    CUDA_VISIBLE_DEVICES=0 python3 scripts/bench_run_huatuo.py --format mcq --shard 0/6 &
    ... etc

Output: results/baselines/huatuogpt-vision-7b__<format>.jsonl
"""
...
def resolve_image_path(probe):
    ...
    if not img_rel:
        return BENCH / "images" / "MVB-0001.jpg"  # placeholder for knowledge_only probes
...
def main():
    ...
    # Override defaults: greedy decoding for reproducibility.
    ...
    for i, probe in enumerate(todo, 1):
        ...
        try:
            t0 = time.time()
            answers = bot.inference(prompt, [str(img)] if img and img.exists() else [])
            answer = answers[0] if isinstance(answers, list) and answers else ""
            # strip echoed prompt template if any
